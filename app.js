...
const redeemPoints=document.createElement('input');redeemPoints.id='redeemPoints';redeemPoints.type='number';redeemPoints.min='10';redeemPoints.step='10';redeemPoints.value='10';redeemPoints.placeholder='Points to redeem';$('redeem').before(redeemPoints);
$('redeem').onclick=async()=>{showMsg($('redeemMsg'),'Submitting...');try{const points=Number(redeemPoints.value);if(!Number.isInteger(points)||points<10||points%10!==0)throw Error('Enter points in multiples of 10.');const data=await customerApi('redeem',{points});showMsg($('redeemMsg'),data.message);await refreshCustomer()}catch(x){showMsg($('redeemMsg'),friendlyError(x),true)}};
...
$('redeem').textContent='Redeem Selected Reward';redeemPoints.previousElementSibling.textContent='Enter any points amount in multiples of 10. Each point equals ₹1.';
