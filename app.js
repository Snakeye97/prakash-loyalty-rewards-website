...
const redeemPoints=document.createElement('input');redeemPoints.id='redeemPoints';redeemPoints.type='number';redeemPoints.min='100';redeemPoints.step='10';redeemPoints.value='100';redeemPoints.placeholder='Points to redeem';$('redeem').before(redeemPoints);
const redeemBill=document.createElement('input');redeemBill.id='redeemBill';redeemBill.type='text';redeemBill.placeholder='Redemption bill number';$('redeem').before(redeemBill);
$('redeem').onclick=async()=>{showMsg($('redeemMsg'),'Submitting...');try{const points=Number(redeemPoints.value),billNumber=redeemBill.value.trim();if(!Number.isInteger(points)||points<100||points%10!==0)throw Error('Enter a minimum of 100 points in multiples of 10.');if(!billNumber)throw Error('Enter the redemption bill number.');const data=await customerApi('redeem',{points,bill_number:billNumber});showMsg($('redeemMsg'),data.message);await refreshCustomer()}catch(x){showMsg($('redeemMsg'),friendlyError(x),true)}};
...
$('redeem').textContent='Redeem Points';redeemPoints.previousElementSibling.textContent='Review your approved bills, then redeem at least 100 available points. Each point equals ₹1.';
