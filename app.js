...
$('redeem').onclick=async()=>{showMsg($('redeemMsg'),'Submitting...');try{const points=Number(redeemPoints.value);if(!Number.isInteger(points)||points<100||points%10!==0)throw Error('Enter a minimum of 100 points in multiples of 10.');const data=await customerApi('redeem',{points});showMsg($('redeemMsg'),data.message);await refreshCustomer()}catch(x){showMsg($('redeemMsg'),friendlyError(x),true)}};
