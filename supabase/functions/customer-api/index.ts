...
Deno.serve(async req=>{
 ...
 try{
  ...
  if(action==='redeem'){
    const points=Number(body.points); if(!Number.isInteger(points)||points<10||points%10!==0) return json({ok:false,error:'Redeem points in multiples of 10.'},400)
 ...
 }catch(e){console.error(e);return json({ok:false,error:e instanceof Error?e.message:'Request failed.'},401)}
})
